...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <!-- Page Heading -->
        <h1 class="h3 mb-4 text-gray-800">Formulário de Conteúdos</h1>
        {{-- <p class="mb-4">Todos os usuários do sistema estão listados aqui nesta página.</p> --}}

        <!-- DataTales Example -->
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Cadastrar/Editar Conteúdos</h6>
            </div>
            <div class="card-body">
                <div class="container">
                    @if (isset($data))
                                {!! Form::model($data, ['method' => 'put', 'enctype' => 'multipart/form-data', 'autocomplete' => false, 'route' => ['dashboard.content.update', $data->id, http_build_query(Request::input())], 'class' => $errors->any() ? 'was-validated' : '', 'novalidate',]) !!}
                            @else
                                {!! Form::open(['method' => 'post', 'enctype' => 'multipart/form-data', 'autocomplete' => false, 'route' => ['dashboard.content.create', http_build_query(Request::input())], 'class' => $errors->any() ? 'was-validated' : '', 'novalidate',]) !!}
                            @endif
                            <div class="row">
                                <div class="col-12">
                                    <div class="col-12">{!! Form::label('section_id', 'Seção de referência', ['class' => 'col-form-label font-weight-bold']) !!}</div>
                                    <div class="col-12"> {!! Form::select('section_id',$sectionsParents, null, ['class' => 'form-control', 'placeholder' => 'Selecione']) !!}
                                        @if (!empty($errors->first('section_id')))
                                            <label class="invalid-feedback d-block">{!! $errors->first('section_id') !!}</label>
                                        @endif
                                    </div>
                                </div>
                                <div class="col-12">
                                ...
                                <div class="col-12">
                                    <div class="col-12">{!! Form::label('site_fold', 'Dobra do site', ['class' => 'col-form-label font-weight-bold']) !!}</div>
                                    <div class="col-12"> {!! Form::select('site_fold',['1' => 'Primeira dobra', '2' => 'Segunda dobra'], null, ['class' => 'form-control', 'placeholder' => 'Selecione']) !!}
                                        @if (!empty($errors->first('site_fold')))
                                            <label class="invalid-feedback d-block">{!! $errors->first('site_fold') !!}</label>
                                        @endif
                                    </div>
                                </div>
                                <div class="col-12">
                                    <div class="col-12">{!! Form::label('author', 'Autor', ['class' => 'col-form-label font-weight-bold']) !!}</div>
                                    <div class="col-12"> {!! Form::text('author', null, ['class' => 'form-control', 'placeholder' => 'Autor' ]) !!}
                                        @if (!empty($errors->first('author')))
                                        <label class="invalid-feedback d-block">{!! $errors->first('author') !!}</label>
                                        @endif
                                    </div>
                                </div>
                                <div class="col-12">
                                ...
                                <div class="col-4">
                                    ...
                                    <div class="col-12"> {!! Form::text('complement', null, ['class' => 'form-control', 'placeholder' => 'Complemento']) !!}
                                        @if (!empty($errors->first('complement')))
                                            <label class="invalid-feedback d-block">{!! $errors->first('complement') !!}</label>
                                        @endif
                                    </div>
                                </div>
                                <div class="col-4">
