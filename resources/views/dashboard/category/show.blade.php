...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <!-- Page Heading -->
        <h1 class="h3 mb-4 text-gray-800">Formulário de Categorias</h1>
        {{-- <p class="mb-4">Todos os usuários do sistema estão listados aqui nesta página.</p> --}}

        <!-- DataTales Example -->
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Cadastrar/Editar Categorias</h6>
            </div>
            <div class="card-body">
                <div class="container">
                    @if (isset($data))
                                {!! Form::model($data, ['method' => 'put', 'enctype' => 'multipart/form-data', 'autocomplete' => false, 'route' => ['dashboard.category.update', $data->id, http_build_query(Request::input())], 'class' => $errors->any() ? 'was-validated' : '', 'novalidate',]) !!}
                            @else
                                {!! Form::open(['method' => 'post', 'enctype' => 'multipart/form-data', 'autocomplete' => false, 'route' => ['dashboard.category.create', http_build_query(Request::input())], 'class' => $errors->any() ? 'was-validated' : '', 'novalidate',]) !!}
                            @endif
                            <div class="row">
                                {{-- <div class="col-12">
                                ...
                                <div class="col-6">
                                    <div class="col-12">{!! Form::label('type_media', 'Tipo', ['class' => 'col-form-label font-weight-bold']) !!}</div>
                                    <div class="col-12"> {!! Form::select('type_media', ['1' => 'Imagem', '2' => 'Video', '3' => 'Documento'], null, ['class' => 'form-control', 'placeholder' => 'Selecione']) !!}
                                        @if (!empty($errors->first('type_media')))
                                            <label class="invalid-feedback d-block">{!! $errors->first('type_media') !!}</label>
                                        @endif
                                    </div>
                                </div>
                                <div class="col-6">
